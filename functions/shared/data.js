 ...
 * 여행에 걸리는 시간 배율.
 *
 * 위 표의 ticks는 "몇 배쯤 걸리는가"의 상대값이고, 실제 소요 시간은 여기서
 * 정한다. 여행은 보내두고 다른 일을 하는 장기 투자라서, 짧으면 배경에 있다는
 * 느낌이 나지 않는다.
 *
 * ⚠️ 배율을 올리면 요리법 습득이 통째로 느려진다. 여행은 한 번에 하나씩만
 * 갈 수 있어서, 10곳을 전부 다녀오는 데 걸리는 시간은
 *   (10+14+16+20+22+28+32+36+40+48) × 배율 × 2초 = 266 × 배율 × 2초
 * 다. 배율 100이면 약 14.8시간으로, 최종 테크 목표(5시간)보다 한참 길다.
 * 요리 관련 업적(왕실 주방장 등)은 한 판에 다 채우기 어려워진다.
 */
export const EXPEDITION_TICK_SCALE = 100;
for (const exp of Object.values(EXPEDITIONS)) exp.ticks *= EXPEDITION_TICK_SCALE;
